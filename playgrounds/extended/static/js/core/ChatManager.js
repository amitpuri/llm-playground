...
class ChatManager {
    ...
    async setApp(app) {
        ...
        // Only populate provider dropdowns if settings are already loaded
    ...
    async populateProviderDropdowns() {
        ...
        if (providerSelect.children.length > 0) {
            ...
            console.log('[ChatManager] Settings default provider:', settingsDefaultProvider);
            console.log('[ChatManager] Available providers:', Array.from(providerSelect.children).map(option => option.value));
            ...
            if (!availableProviders.includes(defaultProvider)) {
                console.log('[ChatManager] Default provider not available, falling back to first available');
                defaultProvider = providerSelect.children[0].value;
            }
            
            console.log('[ChatManager] Final default provider:', defaultProvider);
    ...
    getModelContextWindow(providerKey) {
        ...
        // Get context window based on provider type
        switch (providerKey) {
            case 'openai':
                // OpenAI models have different context windows
                const modelSelect = document.getElementById('model');
                const selectedModel = modelSelect ? modelSelect.value : provider.default_model;
                
                if (selectedModel.includes('gpt-4o')) return 128000;
                if (selectedModel.includes('gpt-4')) return 8192;
                if (selectedModel.includes('gpt-3.5')) return 4096;
                if (selectedModel.includes('gpt-5')) return 128000;
                return 8192; // Default for other OpenAI models
                
            case 'anthropic':
                // Anthropic models
                const anthropicModelSelect = document.getElementById('model');
                const anthropicModel = anthropicModelSelect ? anthropicModelSelect.value : provider.default_model;
                if (anthropicModel.includes('opus')) return 200000;
                if (anthropicModel.includes('sonnet')) return 200000;
                if (anthropicModel.includes('haiku')) return 200000;
                return 200000; // Default for Anthropic
                
            case 'ollama':
                return 4096; // Typical for local models
                
            case 'google':
                return 1000000; // Gemini models have very large context
    ...
    formatJsonResponse(jsonData) {
        ...
        const otherFields = Object.keys(jsonData).filter(key => 
            !['answer', 'used_connectors', 'citations'].includes(key)
        );
